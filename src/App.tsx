...
export default function App() {
  ...
  const [startlistSortColumn, setStartlistSortColumn] =
    useState<string>("jugador");
  const [startlistSortDirection, setStartlistSortDirection] = useState<
    "asc" | "desc"
  >("asc");
  const [startlistPlayerFilter, setStartlistPlayerFilter] = useState<string[]>(
    [],
  );
  const [isStartlistFilterOpen, setIsStartlistFilterOpen] = useState(false);
  ...
  const [isStartlistExpanded, setIsStartlistExpanded] = useState(false);
  const [isStartlistCopying, setIsStartlistCopying] = useState<string | false>(
    false,
  );
  const [isStartlistTextCopying, setIsStartlistTextCopying] = useState(false);
  const [isStartlistTeamsExpanded, setIsStartlistTeamsExpanded] =
    useState(false);
  const [startlistTeamsSortColumn, setStartlistTeamsSortColumn] =
    useState<string>("puntosMedios");
  const [startlistTeamsSortDirection, setStartlistTeamsSortDirection] =
    useState<"asc" | "desc">("desc");
  ...
  const allFilesUploaded = (Object.values(files) as FileState[]).every(
    (f) => f.data !== null,
  );
  ...
  const handleCopyStartlistText = async () => {
    if (!publicStartlistRace || !Array.isArray(files.startlist.data)) return;
    ...
    try {
      ...
      let filteredRows =
        startlistPlayerFilter.length === 0
          ? rows
          : rows.filter((r) => startlistPlayerFilter.includes(r.jugadorName));
      ...
      filteredRows.sort((a, b) => {
        ...
        if (startlistSortColumn === "jugador") {
          ...
          aVal = parseInt(a.dorsal) || 9999;
          bVal = parseInt(b.dorsal) || 9999;
        } else if (startlistSortColumn === "ciclista") {
          aVal = a.ciclista;
          bVal = b.ciclista;
        } else if (startlistSortColumn === "ronda") {
          aVal = parseInt(a.ronda) || 99;
          bVal = parseInt(b.ronda) || 99;
        ...
        if (typeof aVal === "string" && typeof bVal === "string") {
          const compareFn = aVal
            .toLowerCase()
            .localeCompare(bVal.toLowerCase());
          if (compareFn !== 0)
            return startlistSortDirection === "asc" ? compareFn : -compareFn;
        } else {
  ...
  const handleCopyStartlist = async (
    ...
    setIsStartlistCopying(part);
    const restore = expandNodeForCapture(startlistTableRef.current);

    let originalDisplay: string[] = [];
    let trs: NodeListOf<HTMLTableRowElement> | null = null;
    if (part !== "full") {
      const partNum = parseInt(part.substring(1));
      const startIdx =
        chunkStart !== undefined ? chunkStart : (partNum - 1) * 50;
      const endIdx = chunkEnd !== undefined ? chunkEnd : partNum * 50;
      trs = startlistTableRef.current.querySelectorAll("tbody tr");
      trs.forEach((tr, idx) => {
        originalDisplay[idx] = tr.style.display;
    ...
    try {
      ...
      const blobPromise = dataUrlPromise
        .then((url) => {
          if (trs) {
            trs.forEach((tr, idx) => {
              tr.style.display = originalDisplay[idx];
            });
          }
          restore();
          return fetch(url);
      ...
      restore();
      handleDownloadStartlist(part, chunkStart, chunkEnd);
    } finally {
  ...
  const handleDownloadStartlist = async (
    ...
    try {
      ...
      const link = document.createElement("a");
      link.href = dataUrl;
      link.download = `startlist_${publicStartlistRace}_${part}.png`;
      link.click();
    } catch (err: any) {
      if (trs) {
        trs.forEach((tr, idx) => {
    ...
    try {
      ...
      selectedData.resultados.forEach((res: any) => {
        ...
        res.ciclistas.forEach((c: any) => {
          ...
          teamStats[equipoManger].count += 1;
          teamStats[equipoManger].points += displayPuntos;
        });
      });

      const baseRowsData = Object.entries(teamStats).map(([equipo, stat]) => {
        const jugador = teamToPlayerMap[equipo] || equipo;
        const order = playerOrderMap[jugador];
        const equipoVisual = order ? `${equipo} [#${order}]` : equipo;
        return {
          equipo: equipoVisual,
          numCiclistas: stat.count,
          puntos: stat.points,
          puntosMedios:
            stat.count > 0
              ? Math.round((stat.points / stat.count) * 10) / 10
              : 0,
          orden: 0,
        };
      });

      baseRowsData.sort((a, b) => {
      ...
      let currentRank = 1;
      let currentPointsMedios = -1;
      let currentPoints = -1;
      baseRowsData.forEach((r, i) => {
        if (
          r.puntosMedios !== currentPointsMedios ||
          r.puntos !== currentPoints
        ) {
          currentRank = i + 1;
          currentPointsMedios = r.puntosMedios;
          currentPoints = r.puntos;
        }
        r.orden = currentRank;
      });

      const rowsData = [...baseRowsData];

      rowsData.sort((a, b) => {
        let aVal: any = "";
        let bVal: any = "";
        if (startlistTeamsSortColumn === "equipo") {
          aVal = a.equipo;
          bVal = b.equipo;
        } else if (startlistTeamsSortColumn === "numCiclistas") {
          aVal = a.numCiclistas;
          bVal = b.numCiclistas;
        } else if (startlistTeamsSortColumn === "puntos") {
          aVal = a.puntos;
          bVal = b.puntos;
        } else if (startlistTeamsSortColumn === "puntosMedios") {
          aVal = a.puntosMedios;
          bVal = b.puntosMedios;
        }
        ...
        if (typeof aVal === "string" && typeof bVal === "string") {
          const compareFn = aVal
            ...
            .localeCompare(bVal.toLowerCase());
          if (compareFn !== 0)
            return startlistTeamsSortDirection === "asc"
              ? compareFn
              : -compareFn;
        } else {
          if (aVal < bVal)
            return startlistTeamsSortDirection === "asc" ? -1 : 1;
          if (aVal > bVal)
            return startlistTeamsSortDirection === "asc" ? 1 : -1;
        }
        return 0;
  ...
  const handleCopyStartlistTeams = async (part: "full" | string) => {
    ...
    try {
      ...
      if (part !== "full") {
        ...
        options = {
          ...options,
          height: headerHeight + heightPerRow * 25,
          windowHeight: headerHeight + heightPerRow * 25,
          y: headerHeight + (partNum - 1) * heightPerRow * 25,
        };
      }
      ...
      restore();
      handleDownloadStartlistTeams(part);
    } finally {
      setTimeout(() => setIsStartlistTeamsCopying(false), 2000);
    }
  };

  const handleDownloadStartlistTeams = async (part: "full" | string) => {
    if (!startlistTeamsTableRef.current) return;
    const restore = expandNodeForCapture(startlistTeamsTableRef.current);
    try {
      let options: any = {
      ...
      const link = document.createElement("a");
      link.href = dataUrl;
      link.download = `equipos_startlist_${publicStartlistRace}_${part}.png`;
      link.click();
    } catch (err: any) {
      restore();
      console.warn(err?.message);
                        ...
                        <div className="flex justify-between items-center mb-4 bg-white p-3 rounded-lg border border-neutral-200 shadow-sm sticky top-0 z-10">
                          ...
                          <span className="bg-blue-100 text-blue-700 text-xs font-bold px-2 py-1 rounded-full">
                            {parsedStartlist.resultados.reduce(
                              (acc, curr) => acc + curr.ciclistas.length,
                              0,
                            )}{" "}
                            encontrados
                          </span>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 border-b pb-4 gap-4">
                  <div>
                    ...
                    <p className="text-sm text-neutral-500 mt-1">
                      Consulta los ciclistas de la liga participantes en cada
                      carrera.
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    {Array.isArray(files.startlist.data) &&
                      files.startlist.data.length > 0 && (
                        <select
                          ...
                          onChange={(e) =>
                            setPublicStartlistRace(e.target.value)
                          }
                ...
                files.startlist.data.length === 0 ? (
                  ...
                  <div className="space-y-6">
                    {(() => {
                      ...
                      const uniquePlayersInStartlist = [
                        ...new Set(rows.map((r) => r.jugadorName)),
                      ...
                      let filteredRows =
                        startlistPlayerFilter.length === 0
                          ? rows
                          : rows.filter((r) =>
                              startlistPlayerFilter.includes(r.jugadorName),
                            );

                      const nonZeroPoints = filteredRows
                        .map((r) => r.puntos)
                        .filter((p) => p > 0);
                      const maxP =
                        nonZeroPoints.length > 0
                          ? Math.max(...nonZeroPoints)
                          : 0;
                      const minP =
                        nonZeroPoints.length > 0
                          ? Math.min(...nonZeroPoints)
                          : 0;
                      ...
                      filteredRows.sort((a, b) => {
                        ...
                        if (
                          ...
                          const compareFn = aVal
                            ...
                            .localeCompare(bVal.toLowerCase());
                          if (compareFn !== 0)
                            return startlistSortDirection === "asc"
                              ? compareFn
                              : -compareFn;
                        } else {
                          if (aVal < bVal)
                            return startlistSortDirection === "asc" ? -1 : 1;
                          if (aVal > bVal)
                            return startlistSortDirection === "asc" ? 1 : -1;
                        }
                        return 0;
                      ...
                      const handleSort = (col: string) => {
                        if (startlistSortColumn === col) {
                          setStartlistSortDirection((prev) =>
                            prev === "asc" ? "desc" : "asc",
                          );
                        } else {
                      ...
                      rows.forEach((r) => {
                        const equipo =
                          playerTeamMap[r.jugadorName] || r.jugadorName;
                        if (!teamStats[equipo])
                          teamStats[equipo] = { count: 0, points: 0 };
                        teamStats[equipo].count += 1;
                        teamStats[equipo].points += r.puntos;
                      });

                      const baseTeamRows = Object.entries(teamStats).map(
                        ([equipo, stat]) => {
                          const jugador = teamToPlayerMap[equipo] || equipo;
                          const order = playerOrderMap[jugador];
                          ...
                          return {
                            ...
                            puntosMedios:
                              stat.count > 0
                                ? Math.round((stat.points / stat.count) * 10) /
                                  10
                                : 0,
                            orden: 0,
                          };
                      ...
                      // Calcular el orden fijo basado en puntosMedios
                      baseTeamRows.sort((a, b) => {
                        if (b.puntosMedios !== a.puntosMedios)
                          return b.puntosMedios - a.puntosMedios;
                        if (b.puntos !== a.puntos) return b.puntos - a.puntos;
                        return a.equipo.localeCompare(b.equipo);
                      ...
                      teamRows.sort((a, b) => {
                        ...
                        if (startlistTeamsSortColumn === "equipo") {
                          ...
                          startlistTeamsSortColumn === "numCiclistas"
                        ) {
                          aVal = a.numCiclistas;
                          bVal = b.numCiclistas;
                        } else if (startlistTeamsSortColumn === "puntos") {
                          ...
                          startlistTeamsSortColumn === "puntosMedios"
                        ) {
                          aVal = a.puntosMedios;
                          bVal = b.puntosMedios;
                        }
                      ...
                      const handleSortTeams = (col: string) => {
                        if (startlistTeamsSortColumn === col) {
                          setStartlistTeamsSortDirection((prev) =>
                            prev === "asc" ? "desc" : "asc",
                          );
                        } else {
                          setStartlistTeamsSortColumn(col);
                          setStartlistTeamsSortDirection("desc");
                        }
                      };

                      const maxTeamPts = Math.max(
                        ...baseTeamRows.map((r) => r.puntos),
                      );
                      const minTeamPts = Math.min(
                        ...baseTeamRows.map((r) => r.puntos),
                      );
                      const maxTeamPtsMedios = Math.max(
                        ...baseTeamRows.map((r) => r.puntosMedios),
                      );
                      const minTeamPtsMedios = Math.min(
                        ...baseTeamRows.map((r) => r.puntosMedios),
                      );
                      const validCiclistasCnt = baseTeamRows
                        .map((r) => r.numCiclistas)
                        .filter((c) => c > 0);
                      const maxCiclistas =
                        validCiclistasCnt.length > 0
                          ? Math.max(...validCiclistasCnt)
                          : -1;
                      const minCiclistas =
                        validCiclistasCnt.length > 0
                          ? Math.min(...validCiclistasCnt)
                          : -1;

                      const getTeamPointsColorStyle = (val: number) => {
                        if (maxTeamPts === minTeamPts) return {};
                        const ratio =
                          (val - minTeamPts) / (maxTeamPts - minTeamPts);
                        const hue = ratio * 120; // 0 is Red, 120 is Green
                        return { backgroundColor: `hsl(${hue}, 85%, 85%)` };
                      };

                      const getTeamPointsMediosColorStyle = (val: number) => {
                        if (maxTeamPtsMedios === minTeamPtsMedios) return {};
                        const ratio =
                          (val - minTeamPtsMedios) /
                          (maxTeamPtsMedios - minTeamPtsMedios);
                        // 100% Lightness (white) to 70% Lightness (greenish)
                        const lightness = 100 - ratio * 30;
                        return {
                          backgroundColor: `hsl(120, 85%, ${lightness}%)`,
                        };
                      };
                      ...
                      return (
                        <div className="flex flex-col space-y-8">
                          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-neutral-50 p-4 rounded-xl border border-neutral-200">
                            <div className="flex flex-col gap-2 relative">
                              <label className="text-sm font-medium text-neutral-700 whitespace-nowrap">
                                Filtrar por Equipo (selección múltiple):
                              </label>
                              <div className="relative">
                                <button
                                  onClick={() =>
                                    setIsStartlistFilterOpen(
                                      !isStartlistFilterOpen,
                                    )
                                  }
                                  className="w-full sm:w-80 flex items-center justify-between px-3 py-2 bg-white border border-neutral-300 rounded-lg text-sm text-left hover:border-neutral-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                                >
                                  <span className="truncate">
                                    {startlistPlayerFilter.length === 0
                                      ? `Todos los Equipos (${uniquePlayersInStartlist.length})`
                                      : `${startlistPlayerFilter.length} equipos seleccionados`}
                                  </span>
                                  <ChevronDown
                                    className={cn(
                                      "w-4 h-4 text-neutral-400 transition-transform",
                                      isStartlistFilterOpen && "rotate-180",
                                    )}
                                  />
                                </button>
                                {isStartlistFilterOpen && (
                                  <>
                                    <div
                                      className="fixed inset-0 z-40"
                                      onClick={() =>
                                        setIsStartlistFilterOpen(false)
                                      }
                                    />
                                    <div className="absolute top-full left-0 mt-1 w-full sm:w-80 max-h-60 overflow-y-auto bg-white border border-neutral-200 rounded-lg shadow-xl z-50 py-1">
                                      <button
                                        onClick={() => {
                                          setStartlistPlayerFilter([]);
                                          setIsStartlistFilterOpen(false);
                                        }}
                                        className={cn(
                                          "w-full px-3 py-2 text-sm text-left hover:bg-neutral-50 transition-colors",
                                          startlistPlayerFilter.length === 0
                                            ? "font-semibold bg-blue-50/50 text-blue-700"
                                            : "text-neutral-700",
                                        )}
                                      >
                                        Todos los Equipos
                                      </button>
                                      {uniquePlayersInStartlist.map(
                                        (p, idx) => {
                                          const equipoName =
                                            playerTeamMap[p] || p;
                                          const isSelected =
                                            startlistPlayerFilter.includes(p);
                                          return (
                                            <button
                                              key={idx}
                                              onClick={() =>
                                              ...
                                              className={cn(
                                                "w-full flex items-center justify-between px-3 py-2 text-sm text-left hover:bg-neutral-50 transition-colors",
                                                isSelected
                                                  ? "font-semibold bg-blue-50/50 text-blue-700"
                                                  : "text-neutral-700",
                                              )}
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                              <div className="flex items-center gap-1.5 border-r border-neutral-200 pr-3 copy-button-ignore">
                                <button
                                  onClick={() =>
                                    setIsStartlistExpanded(!isStartlistExpanded)
                                  }
                                  className="w-8 h-8 flex items-center justify-center rounded-md border border-neutral-200 bg-white text-neutral-600 hover:bg-neutral-50 hover:text-neutral-900 transition-colors shadow-sm"
                                  title={
                                    isStartlistExpanded
                                ...
                                <button
                                  onClick={() => handleCopyStartlist("full")}
                                  disabled={!!isStartlistCopying}
                                  title="Copiar imagen completa"
                                  className={cn(
                                    "px-2 py-1.5 text-xs font-semibold rounded-md border shadow-sm flex items-center justify-center transition-all text-neutral-600 border-neutral-200 hover:bg-neutral-50 hover:text-neutral-900 w-8",
                                        ...
                                        {chunks.map((chunk, i) => {
                                          ...
                                          return (
                                            <button
                                              ...
                                              disabled={!!isStartlistCopying}
                                              className={cn(
                                                "px-2.5 py-1 text-xs font-semibold rounded-md border shadow-sm flex items-center gap-1.5 transition-all text-neutral-600 border-neutral-200 hover:bg-neutral-50 hover:text-neutral-900 whitespace-nowrap",
                                                isCopyingThis
                                                  ? "bg-green-50 text-green-700 border-green-200"
                                                  : "bg-white",
                                                isStartlistCopying &&
                                ...
                                <button
                                  onClick={handleCopyStartlistText}
                                  disabled={isStartlistTextCopying}
                                  title="Copiar texto"
                                  className={cn(
                                    "px-3 h-8 text-sm font-medium rounded-md border shadow-sm flex items-center justify-center transition-all",
                          ...
                          >
                            {isStartlistExpanded && (
                              <button
                                onClick={() => setIsStartlistExpanded(false)}
                                className="absolute top-6 right-6 p-2 bg-white rounded-full shadow-lg z-10 copy-button-ignore"
                              >
                                <X className="w-6 h-6" />
                              </button>
                              ...
                              <table className="w-full text-sm text-left">
                                <thead className="bg-[#1e293b] text-white border-b border-neutral-200 sticky top-0 z-10">
                                  <tr className="divide-x divide-neutral-600">
                                    <th
                                      className="px-3 py-3 font-semibold cursor-pointer select-none group whitespace-nowrap bg-[#1e293b] hover:bg-[#2d3748]"
                                    ...
                                    <th className="px-3 py-3 font-semibold whitespace-nowrap bg-[#1e293b] text-center">
                                      Debut
                                    </th>
                                    <th
                                      className="px-3 py-3 font-semibold cursor-pointer select-none group whitespace-nowrap bg-[#1e293b] hover:bg-[#2d3748]"
                                ...
                                <tbody className="divide-y divide-neutral-100">
                                  {filteredRows.map((r, i) => {
                                    ...
                                    if (nextRow) {
                                      if (
                                        startlistSortColumn === "jugador" &&
                                        r.jugador !== nextRow.jugador
                                      )
                                        isLastInGroup = true;
                                      if (
                                        startlistSortColumn === "equipo" &&
                                        r.equipo !== nextRow.equipo
                                      )
                                        isLastInGroup = true;
                                      if (
                                        startlistSortColumn === "pais" &&
                                      ...
                                      >
                                        <td
                                          className={cn(
                                            ...
                                            isLastInGroup &&
                                              "border-b-[2px] border-b-neutral-400",
                                          )}
                                        >
                                          {r.jugador}
                                        </td>
                                        <td
                                          className={cn(
                                            "px-2 py-0.5 text-center text-neutral-500 font-mono text-[11px] w-px whitespace-nowrap",
                                            isLastInGroup &&
                                              "border-b-[2px] border-b-neutral-400",
                                          )}
                                        >
                                        ...
                                        <td
                                          className={cn(
                                            "px-2 py-0.5 text-center text-sm leading-none w-px whitespace-nowrap",
                                            isLastInGroup &&
                                              "border-b-[2px] border-b-neutral-400",
                                          )}
                                          title={r.pais}
                                        ...
                                        <td
                                          className={cn(
                                            "px-2 py-0.5 text-center text-neutral-600 text-[11px] whitespace-nowrap w-px",
                                            isLastInGroup &&
                                              "border-b-[2px] border-b-neutral-400",
                                          )}
                                        >
                                        ...
                                        <td
                                          className={cn(
                                            "px-2 py-0.5 text-center font-bold text-[10px] w-px whitespace-nowrap",
                                            r.debut === "Sí"
                                        ...
                                        <td
                                          className={cn(
                                            "px-2 py-0.5 text-center font-mono text-[11px] w-px whitespace-nowrap",
                                            r.dias === 0
                                        ...
                                        <td
                                          className={cn(
                                            "px-2 py-0.5 text-center font-mono text-[11px] font-bold w-px whitespace-nowrap",
                                            getPointsColor(r.puntos),
                                            isLastInGroup &&
                                              "border-b-[2px] border-b-neutral-400",
                                          )}
                                        >
                          ...
                          {/* Segunda tabla: Puntos por Equipos */}
                          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mt-8">
                            <div>
                              ...
                              <p className="text-xs text-neutral-500">
                                Según los ciclistas que participan en la carrera
                              </p>
                            </div>
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                              <div className="flex items-center gap-1.5 border-r border-neutral-200 pr-3 copy-button-ignore">
                                ...
                                <button
                                  onClick={handleCopyStartlistTeamsText}
                                  disabled={isStartlistTeamsTextCopying}
                                  title="Copiar texto"
                                  className={cn(
                                    "px-3 h-8 text-sm font-medium rounded-md border shadow-sm flex items-center justify-center transition-all",
                                ...
                                <button
                                  onClick={() =>
                                    handleDownloadStartlistTeams("full")
                                  }
                                  title="Descargar imagen"
                                  className="px-2 py-1.5 text-xs font-semibold bg-white border border-neutral-200 rounded-md shadow-sm text-neutral-600 hover:bg-neutral-50 flex items-center justify-center transition-colors w-8"
                                >
                          ...
                          >
                            {isStartlistTeamsExpanded && (
                              <button
                                onClick={() =>
                                  setIsStartlistTeamsExpanded(false)
                                }
                                className="absolute top-6 right-6 p-2 bg-white rounded-full shadow-lg z-10 copy-button-ignore"
                              >
                                <X className="w-6 h-6" />
                              ...
                              <table className="w-full text-sm text-left border-r border-neutral-200">
                                <thead className="bg-[#1e293b] text-white border-b border-neutral-200 sticky top-0 z-10">
                                  <tr className="divide-x divide-neutral-600">
                                    ...
                                    <th
                                      className="px-3 py-3 font-semibold cursor-pointer select-none group whitespace-nowrap bg-[#1e293b] hover:bg-[#2d3748]"
                                      onClick={() => handleSortTeams("equipo")}
                                    >
                                      <div className="flex items-center gap-1">
                                        Equipo{" "}
                                        <ArrowUpRight
                                          className={cn(
                                            "w-3 h-3 transition-transform",
                                            startlistTeamsSortColumn ===
                                              "equipo"
                                              ? startlistTeamsSortDirection ===
                                                "asc"
                                                ? "rotate-45"
                                                : "rotate-135"
                                              : "opacity-0",
                                    ...
                                    <th
                                      className="px-3 py-3 font-semibold cursor-pointer select-none group whitespace-nowrap bg-[#1e293b] hover:bg-[#2d3748]"
                                      onClick={() => handleSortTeams("puntos")}
                                    >
                                      <div className="flex items-center justify-center gap-1">
                                        Ptos{" "}
                                        <ArrowUpRight
                                ...
                                <tbody className="divide-y divide-neutral-100">
                                  {teamRows.map((r, i) => (
                                    <tr
                                      ...
                                      className="hover:bg-blue-50/50 transition-colors"
                                    >
                                      <td className="px-2 py-0.5 text-center font-mono text-[11px] text-neutral-500 w-px">
                                        {r.orden}
                                      </td>
                                      <td className="px-2 py-0.5 font-medium text-xs whitespace-nowrap">
                                        {r.equipo}
                                      </td>
                                      <td
                                        className={cn(
                        ...
                        {(() => {
                          const filteredData = files.elecciones.data.filter(
                            (row) => {
                              ...
                              const matchesRound =
                                draftRoundFilter.length === 0 ||
                                draftRoundFilter.includes(
                                  String(getVal(row, "Ronda")),
                                );
                              const matchesTeam =
                                draftTeamFilter.length === 0 ||
                          ...
                          <tbody>
                            {(() => {
                              ...
                              if (pointsRaceSearch.trim()) {
                                const searchLower =
                                  pointsRaceSearch.toLowerCase();
                                const matchedRaces =
                                  files.carreras.data?.filter((r) =>
                                    getVal(r, "Carrera")
                                      ?.toLowerCase()
                          ...
                          {(() => {
                            ...
                            return files.carreras.data
                              ?.filter((r) => {
                                ...
                                if (racesFilter === "finished" && !isFinished)
                                  return false;
                                if (racesFilter === "upcoming" && isFinished)
                                  return false;
                                if (
                                  racesCategoryFilter &&
                                  categoria !== racesCategoryFilter
                                )
                                  return false;
                                if (
                                  racesMonthFilter &&
                              ...
                              .map((r, idx) => {
                                ...
                                return (
                                  <tr
                                    key={idx}
                                    className={cn(
                                      "border-b border-neutral-50 hover:bg-neutral-50",
                                      isFinished
