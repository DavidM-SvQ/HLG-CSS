  ...
  return (
    <div
      ...
      className={cn(
        ...
        isDraftDatosTableExpanded ? "fixed inset-4 z-50 p-6 shadow-2xl m-0" : ""
      )}
    >
      {isDraftDatosTableExpanded && (
        <button
      ...
      <div className="table-responsive-wrapper overflow-auto w-full max-h-[70vh]">
        <table className="min-w-max w-full text-xs text-left border-collapse">
          <thead className="text-[10px] text-neutral-500 uppercase bg-neutral-50/80 backdrop-blur sticky top-0 z-10 shadow-sm border-b border-neutral-200">
            <tr>
              <th
                className="px-4 py-3 cursor-pointer hover:bg-neutral-100 group transition-colors"
                onClick={() => {
                  if (draftDatosSortColumn === "Orden") {
              ...
              <th className="px-4 py-3 font-bold border-l border-neutral-200">Equipo</th>
              <th className="px-3 py-3 border-l border-neutral-200 text-center font-bold">Manager</th>
              {rounds.map((r, i) => (
                <th
                  key={r}
                  className={cn(
              ...
              <th
                className="px-4 py-3 text-right font-black text-blue-700 bg-blue-50/50 border-l border-blue-100 cursor-pointer hover:bg-blue-100/50 transition-colors"
                onClick={() => {
                  if (draftDatosSortColumn === "TOTAL") {
                    setDraftDatosSortDirection(draftDatosSortDirection === "asc" ? "desc" : "asc");
                  } else {
            ...
            {sortedTeams.map((teamName, index) => {
              ...
              return (
                <tr key={teamName} className="hover:bg-blue-50/30 transition-colors group">
                  ...
                  <td className="px-4 py-2 font-bold text-neutral-900 border-l border-neutral-100 truncate max-w-[150px]">
                    {teamName}
                  </td>
                  <td className="px-3 py-2 text-center text-neutral-500 border-l border-neutral-100 text-[10px] uppercase font-bold truncate max-w-[100px]">
                    {player || "-"}
                  </td>
                  {rounds.map((r, i) => {
                  ...
                  <td className="px-4 py-2 text-right font-bold text-blue-700 bg-blue-50/30 border-l border-blue-100">
                    {teamTotal}
                  </td>
                </tr>
              );
...
createPointsTable();
console.log('Fixed DraftPointsTable!');
