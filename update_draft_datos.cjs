...
const draftDatosContent = fs.readFileSync('src/components/tabs/draft/DraftDatos.tsx', 'utf-8');
const lines = draftDatosContent.split('\n');

const imports = lines.slice(0, 48).join('\n');
const startBlock = lines.slice(48, 363).join('\n'); // this closes the filters block up to `<div className="flex flex-wrap items-center gap-2">` and such. Wait, actually I can just recreate the JSX tree.
